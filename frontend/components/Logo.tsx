...
const Logo: React.FC<LogoProps> = ({ textColorClassName = 'text-teal-600', onClick }) => {
    return (
        <div 
            className="flex items-center space-x-2 sm:space-x-4 hover:scale-105 transition-transform duration-200 bg-gradient-to-r from-indigo-800/90 to-purple-800/90 backdrop-blur-sm rounded-lg sm:rounded-xl px-2 sm:px-4 py-2 sm:py-2 shadow-lg border border-indigo-300/30" 
            onClick={onClick} 
        ...
        >
            ...
            <h1 className={`text-sm sm:text-2xl lg:text-3xl font-bold text-white drop-shadow-lg`}>PulseLoopCare</h1>
        </div>
    );
};
